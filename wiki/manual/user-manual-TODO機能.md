...
TODO機能は、日々のタスク・業務を効率的に管理するためのツールです。タスクの作成から完了管理まで、シンプルで直感的な操作で実現できます。トップページのナビゲーションから「TODO」ボタンをクリックすることで、あなたのタスク一覧にアクセスでき、新しいタスク追加、完了マーク、削除などの基本操作がすぐに行えます。
...
1. TODO一覧ページで、入力フィールド（最初の text フィールド）にタスク名を入力します
2. 例：「報告書の作成」「メール返信」「会議資料の準備」など、具体的なタスク内容を記載します
3. 「追加」ボタン（またはEnterキー）をクリックします
4. 新しいタスクが一覧に追加されます
...
2. 削除確認のダイアログが表示されます（ある場合）
3. 「はい」または「OK」ボタンをクリックして削除を確認します
4. タスクが一覧から削除されます
...
- **タスク順序の確認**: TODO一覧は、作成順序または完了状態順に表示されます
- **複数タスクの同時管理**: 複数のタスクを同時に管理でき、スクロールで全タスクを確認できます
- **タスクの編集**: タスク名を変更する場合は、該当タスクをクリックして編集してください（画面デザインによります）

### ページ間のナビゲーション

- **トップページへの移動**: 「トップ」ボタンで、ダッシュボードに戻ります
- **メモ機能への移動**: 「メモ」ボタンで、メモ管理機能に切り替えられます
- **メニューの表示**: ハンバーガーメニュー（☰ mdi-menu）をクリックすると、追加メニューが表示されます

### セッション管理

- **ログイン状態の維持**: ブラウザを閉じるとセッションが終了する場合があります
- **タイムアウト対策**: 長時間操作がない場合は、自動的にログアウトされることがあります
