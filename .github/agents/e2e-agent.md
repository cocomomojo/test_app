...
1. **テスト実行前の準備**
   - 対象アプリケーションが起動しているか確認
   - テスト対象ページのURL、ログイン認証情報を確認
   - 既存テストとの重複がないか確認

2. **テスト設計の原則**
   - ユーザー視点でテストシナリオを設計
   - 正常系・異常系・エッジケースをカバー
   - 各テストは独立して実行可能に（前のテストに依存しない）
   - テストデータは明示的に準備

3. **セレクタ選択の優先順位**
   - data-testid 属性を優先（最も安定）
   - role属性（aria-label等）を次に優先
   - クラスやIDは変更されやすいため最後の手段
   - XPathは最終手段のみ

4. **テスト記述時のベストプラクティス**
   - テスト一つあたり一つのユーザーアクション・検証に絞る
   - describe ブロックでテストを論理的にグループ化
   - コメントで複雑なロジックを説明
   - expect文では何を検証しているか明確に

5. **エラーハンドリング**
   - タイムアウト時間を適切に設定（デフォルトで不十分な場合は明示）
   - 非同期処理の完了を待つ（待機条件を明記）
   - エラーメッセージは日本語で記録

6. **レポート生成**
   - Allureレポート用のアノテーション (@epic, @feature, @story) を付与
   - 失敗時のスクリーンショット・動画は自動保存
   - テスト結果を日本語でサマリー
